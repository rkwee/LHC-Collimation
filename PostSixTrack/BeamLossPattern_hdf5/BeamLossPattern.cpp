...
int main (int argc, char* argv[])
{
  if (argc != 5){
    ...
    cout<<"  -> " << argv[0] <<" input.dat output.txt ApertureFile.txt SurveyFile.txt <-"<<endl;
    exit(1);
  }
  ...
  // WARNING: Take care, setting _X_ means your survey files must be updated for almost
  //  all changes in the optics (like turning on/off xangles, separation etc)
  int _X_ = 0;       // Flag for using crossing

  int _SaveLost_ = 0; //  Save the trajectories of losdt particles (last turn) -- rkwee: BROKEN it is stuck in the loop
  ...
  cout<<setw(50)<<"Survey file: "<<SurFile<<endl;
  ...
  //  return 0;
  if ( _X_ ) {
    cout << "WARNING: Make sure the Survey file is accurate!" << endl;
  }
  //////////////////////////////
  ...
  cout<<"Reading: "<<SurFile<<endl<<endl;
  LHC_sur.LoadLHC_Crossing_XP( SurFile );
  ...
  ReadTracks rtrcks(File);
  while (rtrcks.getNextTrack(n_t,n_tu,s_t,x_t,xp_t,y_t,yp_t,en_t,n_h)==0) {
  ...
  if ( _SaveLost_ ) {
    // Write out the trajectories of lost particles (in one single file)
    ...
    //
    out_part.open("LostParticles.dat", ios_base::out);
    //
    in.open(File.c_str(), ios_base::in);
    if (!in){
