 ...
 *
 * Required inputs:
 * 1) Coll_Scatter.dat output file from SixTrack
 ...
 * to compile: g++ -o CleanInelastic_2013-08-14 CleanInelastic_2013-08-14.cpp
...
int main (int argc, char* argv[])
{
  ...
  int NCollMax = 100;  // Maximum number of collimators
  ...
  vector<double> ANGLE_inel, POS_inel, X_inel, XP,
    Y_inel, YP, dp;
  ...
  for (int i = 0; i < Length.size(); i++) {
    if ( Length[i] > 0 ) {
      j = 0;
      while ( CollName[j] != Name[i] && j < CollName.size() )
	j++;
      if ( j == CollName.size() ) {
  ...
  in.getline(c_str,256); // Skip the header line
  while (1){
    in>>i1>>i2>>i3>>i4>>d1>>d2>>d3;
    if (!in.good())
      break;
    NCOLL_inel.push_back(i1);
    NTURN_inel.push_back(i2);
    NPART_inel.push_back(i3);
    FLAG_inel.push_back(i4);
    dp.push_back(d1);
    XP.push_back(d2);
    YP.push_back(d3);
  }
  ...
  FILE *fp1, *fp2;
  fp1=fopen("Coll_Scatter_real.dat","w");
  fp2=fopen("Coll_Scatter_fake.dat","w");
  fprintf(fp1,"%s\n"," #1=icoll, 2=iturn, 3=np, 4=nabs (1:Nuclear-Inelastic,2:Nucle ar-Elastic,3:pp-Elastic,4:Single-Diffractive,5:Coulomb)");
  fprintf(fp2,"%s\n"," #1=icoll, 2=iturn, 3=np, 4=nabs (1:Nuclear-Inelastic,2:Nucle ar-Elastic,3:pp-Elastic,4:Single-Diffractive,5:Coulomb)");

  for (int i = 0; i < NCOLL_inel.size(); i++){
    n_p = NPART_inel[i];
    // RB: bug fix
    if ( !( NPART_lost[n_p] == 1 && //particle is listed as lost in LPI
	    ( NTURN_lost[n_p] < NTURN_inel[i] || // particle is listed as lost in LPI on a previous turn, OR 
	      (NTURN_lost[n_p] == NTURN_inel[i] && POS_lost[n_p] <= Position[NCOLL_inel[i]]))) )  // particle listed as lost in LPI on the same turn but at smaller s // end RB bug fix
      {
	fprintf(fp1, "%4d %5d %5d %2d %14.5e %14.5e %14.5e\n", 
		NCOLL_inel[i],NTURN_inel[i],NPART_inel[i],FLAG_inel[i],dp[i],XP[i],YP[i]);
      } 
    else {
      fprintf(fp2, "%4d %5d %5d %2d %14.5e %14.5e %14.5e\n",
	      NCOLL_inel[i],NTURN_inel[i],NPART_inel[i],FLAG_inel[i],dp[i],XP[i],YP[i]);
    }
  }
