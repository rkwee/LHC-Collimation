...
* OLD      The file should contain x y z [in cm] x' y' [angles in rad] *
* NEW from RB: file should contain x y z [in cm] CX CY [dir cosines]   *
...
* Cosines
************
* old version form FC - presuming we have x' and y' in file
c$$$      DCX=TAN(UIN(N))
c$$$      DCY=TAN(VIN(N))
c$$$      FNORM = SQRT(DCX**2+DCY**2+ONEONE)
c$$$      TXI    = DCX / FNORM
c$$$      TYI    = DCY / FNORM
* new version RB: presuming we have already the cosines in the file
      TXI    = UIN(N)
      TYI    = VIN(N)
...
* check the material of the region where particle starts. If not vacuum, issue warning
      IF (MMAT.NE.2) THEN
        NVACUUM = NVACUUM + 1
        WRITE(LUNERR,*) "*******************************************"
     ...
     +                  " DIR=",TXI,TYI,TZI," REG=",MREG
        WRITE(LUNERR,*) "  Material is not VACUUM"
        WRITE(LUNOUT,*) "  Material is not VACUUM"

        IF (NVACUUM.GT.20) STOP 'TOO MANY INTERACTIONS OUTSIDE VACUUM'
      ENDIF

* reset material on which inelastic interaction is forced to WHAT(2)
      MMAT=NINT(WHASOU(2))

*      write(88,*) XXX,YYY,ZZZ,tStart
*      write(88,*) WHASOU(1),MMAT
