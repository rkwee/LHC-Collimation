...
* Constant Field
         IF (FILENAME.EQ.'CONST') THEN
             DO I=NREGFROM,NREGTO,NSTEP
                 CALL MAGCNST(I,FIELD,WHAT(1),WHAT(2),WHAT(3))
             ENDDO
         ELSE
...
*===magcnst============================================================*
*
      SUBROUTINE MAGCNST( NREG, B, BX, BY, BZ )
...
* Default Values
      NFREG (NREG)   = NFIELD
      FFILE (NFIELD) = 'Internal'
      IFTYP (NFIELD) = NFTCONST
...
* Print Fields
     ...
     +                              ' ',IFTYP(NF)
         IF (IFTYP(NF).EQ.NFTCONST) GOTO 10
