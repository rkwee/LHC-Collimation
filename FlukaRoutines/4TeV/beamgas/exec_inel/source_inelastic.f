...
*----------------------------------------------------------------------*
*                                                                      *
*     Inelastic interaction sampling                                   *
*                                                                      *
...
*  |  *** The following 3 cards are mandatory ***
         ...
         LUSSRC = .TRUE.
*  |  *** User initialization ***
         WRITE (LUNOUT,*) 'Source routine forcing inelastic interact.'
*      
         IEVENT = 0
         BEAWEI = ONEONE
      END IF
...
*  |  Compute the inelastic sigma:
        ...
        WRITE (LUNOUT,*)' Inelastic interaction cross section: ',
     &                               SIGMCI, ' (1/cm) in', MMAT
*  |  +----------------------------------------------------------------*
...
*      WRITE (92,*)' # Event No ',IEVENT,', particles in stack: ',NP
...
*  |  Calculate the total kinetic energy of the primaries: don't change
     ...
     &             + AMDISC (ILOFLK(NPFLKA)) ) * WTFLK (NPFLKA)
         ...
         END IF
*  |  Filter particles assuming:
*  |     1) particle is a proton ( KPART (KP) = 1 )
*  |     2) kinetic energy is 95 perc of incident kinetic energy 
*  |     3) dummy criterion for direction cosines (CXR,CYR,CZR)
*  |        (replace with meaningful one)
         IF( ( KPART (KP) .EQ. 1 ) .AND. 
     &       ( TKI(KP) .GT. 0.9 * EKE ) ) THEN
*     &       ( (CXR(KP)*CXR(KP) + CYR(KP)*CYR(KP)) .LT. 1.0D-07) ) THEN
*  |
